...
class EnsembleGPOptimizer(EnsembleGP):
    """
    EnsembleGPOptimizer class: Given data, this class can determine which
    ...
    Example:
        obj = EnsembleGPOptimizer(3,[[0,10],[0,10],[0,10]])
        obj.tell(x,y)
        obj.init_gp(...)
    ...
    def evaluate_acquisition_function(self,
        ...
        -----------
        x: 1d numpy array.
        acquisition_function: a function that takes the posterior and produces a svalar that is maximized 
                              in search if new optimal experiments

        Optional Parameters:
        --------------------
        cost_function = None
        origin:                default = None, only important for cost 
                               considerations, mandatory if costs are important
...
######################################################################################
######################################################################################
class EnsemblefvGPOptimizer(fvGP, GPOptimizer):
    """
    ...
    def __init__(
        ...
        type help(gp_optimizer) for help
        """
        raise Exception("Not implemented yes, EXIT!")
