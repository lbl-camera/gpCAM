...
class GPOptimizer(GP):
    ...
    gp_kernel_function : Callable, optional
        ...
        `gpcam.GPOptimizer` instance. The default is a stationary anisotropic kernel
        (`fvgp.gp.default_kernel`).
    gp_mean_function : Callable, optional
        ...
        provided,
        `fvgp.gp.default_mean_function` is used.
    ...
    cost_function : Callable, optional
        ...
        are an `origin` (np.ndarray of size V x D), `x` (np.ndarray of size V x D), and the value of `cost_func_params`;
        ...
        returned cost to determine the next measurement point. The default in no-op.
    ...
    def __init__(
        ...
        if init_hyperparameters is None: raise Exception("The syntax to initialize the GPOptimizer has recently changed. Please consult the latest version of the docs at www.gpcam.readthedocs.io")
        self.iput_dim = input_space_dimension
        if x_data is None: x_data = np.empty((1, self.iput_dim))
        if y_data is None: y_data = self.y_data = np.empty((1))
        if variances is None: variances = self.variances = np.empty((1))
...
class fvGPOptimizer(fvGP):
    ...
    def ask(self, position=None, n=1, acquisition_function,
            bounds=None,
            method="global",
            pop_size=20,
            max_iter=20,
