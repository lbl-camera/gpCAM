...
- **Python >= 3.10**; CI matrix is 3.10–3.14 (`.github/workflows/gpCAM-CI.yml`). Dependencies are declared as **ranges**, not `~=` pins, because no single scipy/numpy release spans that span; pip resolves per interpreter (3.10 → scipy 1.13 / dask 2024.1; 3.11+ → the 2025-era stack). Those versions are effectively chosen upstream: **hgdl `~=`-pins the whole scientific stack**, so hgdl — reached through fvgp — decides what a given Python gets. Raising the `fvgp` floor is what unlocks a new Python here: 4.8.5 declared `requires_python >= 3.11`, so no gpCAM metadata could have made 3.10 installable before 4.8.6. A second workflow refreshes Context7 docs on version tags.
- The whole test suite is `tests/test_gpCAM.py` — 38 tests, at **100% coverage**; keep it there. Most of the additions are fast and client-free: the lazy-init accessors, `ask()`'s argument rewriting, `surrogate_model`'s validation and lookahead helpers, and the transformed optimizers' `_forward_deriv`. `test_basic_1task`, `test_basic_multi_task`, and `test_acq_funcs` take the `client` fixture from `distributed.utils_test` and spin up a **local Dask cluster**, so they are slow and start real worker processes; the rest (`test_optimizers`, `test_pickle`, `test_transformed_gp`, and the acquisition regression tests) are fast and client-free — prefer those for quick iteration. Multiprocessing entrypoint guards (`if __name__ == "__main__":`) matter in user-facing example scripts.
...
The repo is on the **8.4.x line** (latest tag `8.4.2`, `fvgp ~= 4.8.5`), which is now the recommended one — the beta warning was retired once 8.4 proved solid. Users on the older **8.3.9** may still turn up, and HISTORY.rst plus the README upgrade note are the migration reference. 8.4.1 → 8.4.2 was a docs/skills-only release (identical wheel), so a HISTORY entry does not imply library changes. Constructor/method kwargs renamed in 8.4:
...
**gpCAM is a thin Bayesian-optimization / autonomous-data-acquisition layer on top of the [`fvgp`](https://github.com/lbl-camera/fvgp) package** (`fvgp ~= 4.8.6` in `pyproject.toml` — `~=` allows any later 4.8.z but not 4.9). The floor is 4.8.6 because that is the release carrying `gp2Scale_distribution`; forwarding a keyword an older fvGP does not have is a `TypeError`, not a graceful no-op, so the floor must move whenever a new constructor argument is passed through. The actual GP math — kernels, hyperparameter training, posterior evaluation, MCMC, deep kernels — lives in `fvgp`. gpCAM adds the `ask`/`tell`/`train`/`optimize` loop and acquisition-function optimization on top.
...
`ask()` returns `{'x': ..., 'f_a(x)': ..., 'opt_obj': ...}` (`opt_obj` is non-`None` only for `hgdl`). Non-Euclidean / mixed spaces are supported by passing `input_set` as a **list of candidates** instead of a bounds array — but that path bypasses the optimizers entirely: it evaluates every candidate and sorts, so `method`, `x0`, `pop_size`, `max_iter`, and `constraints` are all ignored.

**Batch selection from a candidate list** splits on whether the acquisition can score a set. `SET_VALUED_ACQUISITION_FUNCTIONS` in `surrogate_model.py` names the two that can — `"total correlation"` and `"relative information entropy"`, the only built-ins returning one scalar for a whole set. For those with `n > 1`, `_greedy_batch_from_candidates` picks the batch jointly by greedy forward selection (best single candidate, then the one that best complements it) and `'f_a(x)'` is **one value for the set**, not one per point. For every other acquisition — including any callable, which gpCAM cannot introspect — `n` still just truncates the sorted list, and `ask()` now warns that the points are not mutually optimal. That warning matters: the point-wise top-`n` are routinely near-duplicates, since nothing stops the best scorers from clustering.

The two set-valued acquisitions are both KL divergences but answer different questions. `"total correlation"` compares the joint distribution over data ∪ candidates against the same thing with the data↔candidate cross-covariance dropped and the candidates decorrelated — so it measures redundancy, and (negated, as gpCAM does) prefers points novel relative to the data *and* to each other. `"relative information entropy"` compares prior to posterior at the candidates only — so it measures how much the existing data has already taught you there, and prefers untouched regions. For "which experiments should I run together", `"total correlation"` is the more direct criterion; RIE spreads points only as a side effect of them all being unexplored, so it tolerates mutually redundant neighbors.
