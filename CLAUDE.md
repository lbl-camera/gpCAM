...
- **Python >= 3.11**; CI matrix is 3.11 and 3.12 (`.github/workflows/gpCAM-CI.yml`). fvGP now supports 3.10–3.14 with range-based dependency pins; gpCAM can follow once 4.8.6 is released, but not before — a released fvgp that declares `requires_python >= 3.11` makes a 3.10 install impossible no matter what gpCAM's own metadata says. The same `~=` pins that blocked fvGP (`scipy ~= 1.16.0` needs >= 3.11 and has no cp314 wheel) are still here. A second workflow refreshes Context7 docs on version tags.
